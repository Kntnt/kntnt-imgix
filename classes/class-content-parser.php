...
class Content_Parser {
  ...
  public function run() {
    ...
    if (Plugin::option('imgix-domain')) {
      ...
      // Translate URL:s retunred by `wp_get_attachment_image_src()` if the
      // filter `kntnt_imgix_translate_image_srcset` returns true. Default:
      // true.
      ...
      // Translate URL:s retunred by `wp_get_attachment_image_src()` if the
      // filter `kntnt_imgix_translate_attachment_image_src` returns true.
      // Default: true.
      ...
      // "the content" output if the filter `kntnt_imgix_parse` returns 'fast'.
      // Default by settings page.
      ...
      // Search and replace local image URLs with dito Imgix in WordPress'
      // final output of content if `kntnt_imgix_parse` returns 'thoroughly'.
      // Default by settings page.
