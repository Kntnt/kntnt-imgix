...
class Image_Editor_Imgix extends \WP_Image_Editor {
  ...
  private static $supported_file_suffixes = array(
    'jpg',
    'jpeg',
    'jxr',
    'jp2',
  ...
  public static function test($args = array()) {
    if (!Plugin::option('imgix-domain')) return false;
    $regex = '~' . Plugin::uploads_dir_rel_site() . '/(?:\d{4}/\d{2}/)?[^/]+\.(?:' . implode("|", self::$supported_file_suffixes) . ')$~i';
    if (isset($args['path']) && !preg_match($regex, $args['path'])) return false;
    return true;
  }
  ...
  public function load() {
    ...
    if (!is_readable($this->file)) return new \WP_Error('error_loading_image', __("File doesn't exist?"), $this->file);

    // Get information about the image.
    $size = @getimagesize($this->file);
    if (!$size) return new \WP_Error('invalid_image', __('Could not read image size.'), $this->file);
  ...
  private function _resize($max_w, $max_h, $crop) {
    ...
    if (!$dims) return new \WP_Error('error_getting_dimensions', 'Could not calculate resized image dimensions', $this->file);
  ...
  public function multi_resize($sizes) {
    ...
    foreach ($sizes as $size => $dst_size) {
      ...
      if (($dst_size['width'] == $org_size['width']) && ($dst_size['height'] == $org_size['height'])) continue;

      // If images has no width or height, there is not much to do.
      if (!isset($dst_size['width']) && !isset($dst_size['height']))  continue;

      // Add defaults for missing parameters.
      if (!isset($dst_size['width'])) $dst_size['width'] = null;
      if (!isset($dst_size['height'])) $dst_size['height'] = null;
      if (!isset($dst_size['crop'])) $dst_size['crop'] = false;
  ...
  public function fetch($out = null) {
    ...
    try {
      ...
      // Initalize cURL
      $ch = curl_init();
      if ($ch === false) return new \WP_Error('error_loading_image', __("Can't write to file."), $this->file);
      ...
      if ($out) {
        $fp = fopen($out, 'wb');
        if ($fp === false) return new \WP_Error('error_loading_image', __("Can't read from $url"), $this->file);
        curl_setopt($ch, CURLOPT_FILE, $fp);
      }

      // Read and write the image.
      curl_exec($ch);
      if (curl_error($ch)) return new \WP_Error('error_loading_image', __("Failed writing image to file."), $this->file);
